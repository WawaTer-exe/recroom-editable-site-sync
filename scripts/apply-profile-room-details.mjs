...
const roomCards = linkedRooms.map((room) => {
  ...
  const preview = unique([...(roomGallery.get(slug) || []), metadata?.coverUrl])[0];
  const previewMarkup = preview ? `<img class="archive-room-thumb" loading="lazy" src="${escapeHtml(preview)}" alt="${escapeHtml(metadata?.title || slug)} room preview">` : `<div class="archive-room-thumb empty">No preview captured</div>`;
  const stats = `<div class="archive-room-stats"><span>Cheers ${escapeHtml(metadata?.cheerCount ? Number(metadata.cheerCount).toLocaleString() : "—")}</span><span>Visits ${escapeHtml(metadata?.visitCount ? Number(metadata.visitCount).toLocaleString() : "—")}</span></div>`;
  return `<a class="archive-room-link" href="../../room/${encodeURIComponent(slug)}/">${previewMarkup}<span class="archive-room-copy"><strong>${escapeHtml(metadata?.title || slug)}</strong><span>${escapeHtml(description)}</span><small>@${escapeHtml(creator)}</small>${stats}</span></a>`;
}).join("");
